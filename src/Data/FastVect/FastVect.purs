...
import Data.FastVect.Add (class AddX)
...
append ∷ ∀ m n elem carry sum m_plus_n. Cons carry sum m_plus_n ⇒ AddX m n carry sum ⇒ Vect m elem → Vect n elem → Vect m_plus_n elem
append (Vect _ xs) (Vect _ ys) = Vect (Proxy ∷ Proxy m_plus_n) (xs <> ys)

drop ∷ ∀ m n elem carry sum m_plus_n. ToInt m ⇒ Cons carry sum m_plus_n ⇒ AddX m n carry sum ⇒ Proxy m → Vect m_plus_n elem → Vect n elem
drop proxy (Vect _ xs) = Vect (Proxy ∷ Proxy n) (A.drop (toInt proxy) xs)

take ∷ ∀ m n elem carry sum m_plus_n. ToInt m ⇒ Cons carry sum m_plus_n ⇒ AddX m n carry sum ⇒ Proxy m → Vect m_plus_n elem → Vect m elem
